...
# ── it against the 30 known feature names to pre-fill the sidebar fields ──
def extract_text_from_upload(uploaded_file):
    ...
    if uploaded_file.type == "application/pdf":
        try:
            ...
            pass
        if len(text.strip()) < 20:  # likely a scanned PDF with no selectable text
            try:
...
def parse_feature_values(text, feature_names):
    ...
    the nearest following number as its value. Returns {name: value} for
    whatever it could confidently match."""
    results = {}
...
uploaded_report = st.sidebar.file_uploader(
    "Upload a lab report (PDF or photo)", type=["pdf", "png", "jpg", "jpeg"]
)
st.sidebar.caption(
...
if uploaded_report is not None:
    ...
    if extracted:
        for name, val in extracted.items():
            st.session_state[name] = val  # pre-fills the matching number_input below
        ...
        st.sidebar.error(
            ...
            "format doesn't align with the 30 expected measurements, or the "
